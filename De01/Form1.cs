...
namespace De01
{
    public partial class Form1 : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                int rowIndex = FindStudentByName(searchName);
                if (rowIndex != -1)
                {
                    dgvStudent.Rows[rowIndex].Selected = true;
                ...
                {
                    MessageBox.Show("Không tìm thấy sinh viên có tên '" + searchName + "'", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
                MessageBox.Show("Vui lòng nhập tên sinh viên cần tìm kiếm", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private int FindStudentByName(string name)
        {
            for (int i = 0; i < dgvStudent.Rows.Count; i++)
            {    
